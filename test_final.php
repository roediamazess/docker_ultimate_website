<?php
session_start();
require_once 'db.php';
require_once 'access_control.php';
require_login();

header('Content-Type: text/html; charset=utf-8');

echo "<h2>Test Final - Check Data Display</h2>";

try {
    // Ambil data dari database
    $sql = "SELECT COALESCE(user_id,'') AS user_id, pic_name, day, value, 
            CASE WHEN ontime IS NULL THEN false ELSE ontime END as ontime,
            CASE WHEN late IS NULL THEN false ELSE late END as late,
            note FROM jobsheet ORDER BY pic_name, day";
    
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    echo "<p>Total records: " . count($rows) . "</p>";
    
    // Cek data Akbar
    $akbarData = array_filter($rows, function($row) {
        return $row['pic_name'] === 'Akbar';
    });
    
    echo "<p>Akbar records: " . count($akbarData) . "</p>";
    
    if (count($akbarData) > 0) {
        echo "<h3>Akbar Data:</h3>";
        echo "<table border='1' style='border-collapse: collapse;'>";
        echo "<tr><th>pic_name</th><th>day</th><th>value</th><th>ontime</th><th>late</th></tr>";
        
        foreach ($akbarData as $row) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['pic_name']) . "</td>";
            echo "<td>" . htmlspecialchars($row['day']) . "</td>";
            echo "<td>" . htmlspecialchars($row['value']) . "</td>";
            echo "<td>" . ($row['ontime'] ? 'true' : 'false') . "</td>";
            echo "<td>" . ($row['late'] ? 'true' : 'false') . "</td>";
            echo "</tr>";
        }
        echo "</table>";
        
        echo "<h3>JSON Response:</h3>";
        $jsonResponse = json_encode($akbarData, JSON_UNESCAPED_UNICODE);
        echo "<pre>" . htmlspecialchars($jsonResponse) . "</pre>";
    }
    
    echo "<h3>All Data:</h3>";
    echo "<table border='1' style='border-collapse: collapse;'>";
    echo "<tr><th>pic_name</th><th>day</th><th>value</th><th>ontime</th><th>late</th></tr>";
    
    foreach ($rows as $row) {
        echo "<tr>";
        echo "<td>" . htmlspecialchars($row['pic_name']) . "</td>";
        echo "<td>" . htmlspecialchars($row['day']) . "</td>";
        echo "<td>" . htmlspecialchars($row['value']) . "</td>";
        echo "<td>" . ($row['ontime'] ? 'true' : 'false') . "</td>";
        echo "<td>" . ($row['late'] ? 'true' : 'false') . "</td>";
        echo "</tr>";
    }
    echo "</table>";
    
} catch (Throwable $e) {
    echo "<p style='color: red;'>Error: " . htmlspecialchars($e->getMessage()) . "</p>";
}
?>
