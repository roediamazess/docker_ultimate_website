...
<?php include './partials/layouts/layoutHorizontal.php'; ?>
    ...
    <div class="container-fluid">
        <!-- Status Tabs (order: Running, Scheduled, Document, Document Check, Done, Cancel, Rejected, All Status) -->
        <style>
            .status-tab { border:none; padding:8px 14px; border-radius:10px; font-weight:600; }
            .status-tab.active { color:#fff; background: linear-gradient(90deg,#6f74f6,#a37cf5); box-shadow: 0 6px 18px rgba(111,116,246,.35); }
            .status-tab.inactive { color:#e5e7eb; background:#6b7280; }
            .status-tabs-wrap { display:flex; gap:10px; justify-content:flex-end; margin-bottom:14px; }
            /* Dark theme: make tabs more subtle like Activities */
        ...
        <style>
            ...
            /* Modal detail cards (PIC rows) */
            .custom-modal .detail-row { background:#f8fafc; border:1px solid #e5e7eb; border-radius:12px; }

            /* Dark theme alignment with Activities */
            [data-theme="dark"] .custom-modal { background:#1f2937; border:1px solid #374151; }
            [data-theme="dark"] .custom-modal-header { background: linear-gradient(135deg, #0f172a 0%, #111827 100%); color:#e5e7eb; border-bottom:1px solid #374151; }
            [data-theme="dark"] .custom-modal-footer { background:#0b1220; border-top:1px solid #1f2937; }
            [data-theme="dark"] .custom-modal .detail-row { background:#0b1220; border:1px solid #1f2937; }
            [data-theme="dark"] .custom-modal input:disabled,
            [data-theme="dark"] .custom-modal select:disabled,
            [data-theme="dark"] .custom-modal textarea:disabled { background:#0b1220; color:#9ca3af; border-color:#374151; opacity:1; }
            [data-theme="dark"] .badge.bg-secondary { background:#374151 !important; color:#e5e7eb !important; }
            [data-theme="dark"] .btn-outline-success { border-color:#059669; color:#a7f3d0; }
            [data-theme="dark"] .btn-outline-warning { border-color:#d97706; color:#fde68a; }
        </style>
        ...
        <div class="card" id="projectsListCard">
            ...
            <div class="card-body">
                <style>
                    ...
                    /* Dark theme parity with Activities list */
                    [data-theme="dark"] #projectsListCard .filter-section{ background:#0b1220; border-color:#1f2937; }
                    [data-theme="dark"] #projectsListCard .filter-label{ color:#e5e7eb; }
                    [data-theme="dark"] #projectsListCard .form-select,
                    [data-theme="dark"] #projectsListCard .form-control{ background:#111827; border-color:#374151; color:#e5e7eb; }
                    [data-theme="dark"] #projectsListCard .form-select:focus,
                    [data-theme="dark"] #projectsListCard .form-control:focus{ border-color:#667eea; box-shadow:0 0 0 3px rgba(102,126,234,0.12); }
                    [data-theme="dark"] #projectsListCard .btn-reset{ background:#1f2937; color:#e5e7eb; border-color:#374151; }
                    [data-theme="dark"] #projectsListCard .btn-reset:hover{ background:#374151; }
                    [data-theme="dark"] .table .table-header{ background: linear-gradient(135deg,#0f172a 0%, #111827 100%) !important; color:#e5e7eb !important; box-shadow:0 2px 8px rgba(2,6,23,.4) !important; }
                        ...
                        [data-theme="dark"] .project-detail-row td {
                            ...
                            box-shadow: 0 2px 8px rgba(2,6,23,0.35) inset;
                        }
                </style>
                ...
                <div class="filter-section">
                    <form method="get" class="filter-form">
                        ...
                        <div class="d-flex gap-2" style="margin-top:12px;">
                            ...
                            <button type="button" class="btn-apply btn btn-sm btn-primary-600" onclick="openAddModal()">Add Project</button>
                        </div>
                    </form>
                </div>
  ...
  <div class="custom-modal" style="max-width:700px;">
    ...
    <div class="custom-modal-footer">
      <button type="button" class="custom-btn custom-btn-secondary" onclick="document.getElementById('customerLookupModal').style.display='none'">Close</button>
        ...
        // Enable ESC and outside-click to close modal (ESc UX)
        (function setupProjectModalDismiss(){
            if (window.__projectModalDismissSetup) return; 
            window.__projectModalDismissSetup = true;
            ...
            document.addEventListener('keydown', function(e){
                if (e.key === 'Escape' || e.key === 'Esc'){
                    const overlay = document.getElementById('projectModal');
                    if (overlay && overlay.style.display === 'flex') { closeProjectModal(); }
                }
            });
        })();
