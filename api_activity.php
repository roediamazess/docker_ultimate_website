...
try {
    if (isset($_GET['id'])) {
        $id = (int)$_GET['id'];
        $stmt = $pdo->prepare('SELECT a.id, a.project_id, a.no, a.information_date, a.user_position, a.department, a.application, a.type, a.description, a.action_solution, a.due_date, a.status, a.cnc_number, a.priority, a.customer, a.project, a.created_by, u.display_name AS created_by_name, a.created_at FROM activities a LEFT JOIN users u ON u.id = a.created_by WHERE a.id = ?');
        $stmt->execute([$id]);
